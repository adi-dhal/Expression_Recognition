...
from NN import trainNN, use_NN,use_SVM,trainSVM
import sys
...
def process_image(img_name,flag):
	...
	vector.append(eye_open(x_coord[36:42],y_coord[36:42]))
	vector.append(eye_open(x_coord[42:48],y_coord[36:42]))
	vector.append(mouth_open(x_coord[60:68],y_coord[60:68]))
	if flag == 1:
		use_NN(vector)
	else:
		use_SVM(vector)
...
if len(sys.argv) == 2:
	...
	if  int(sys.argv[1]) == 1:
		process_image(sys.argv[2],1)
	elif  int(sys.argv[1]) == 2:
		process_image(sys.argv[2],2)
