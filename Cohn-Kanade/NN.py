...
def train_neural_network(x):
	...
	optimizer = tf.train.AdamOptimizer().minimize(cost)						#Optimizer
	hm_epochs = 50
...
def use_SVM(sample):
	extract_data(2)
	clf.fit(np.array(train_data), np.array(train_lab))	# Train Svm
	print clf.predict(np.array(sample).reshape(1,-1))	# Predict Class with SVM
def use_NN(sample):
	prediction = neural_network_model(x)
	with tf.Session() as sess:
		saver = tf.train.Saver()
